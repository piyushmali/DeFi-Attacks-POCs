...
async function main() {
  ...
  const initialAttackerB = await tokenB.balanceOf(attacker.address);
  ...
  // Attacker frontruns with a smaller swap
  ...
  // Calculate price impact and attacker's profit in TKB
  const finalAttackerB = await tokenB.balanceOf(attacker.address);
  const profitTKB = finalAttackerB - initialAttackerB;
  const priceAfterFrontrun = await dex.getPrice();
  const priceImpact = ((Number(ethers.formatEther(priceAfterFrontrun)) - Number(ethers.formatEther(initialPrice))) / Number(ethers.formatEther(initialPrice))) * 100;
  console.log(`\nPrice after frontrun: ${ethers.formatEther(priceAfterFrontrun)}`);
  console.log(`Price impact: ${priceImpact.toFixed(2)}%`);
  console.log(`Attacker's TKB profit: ${ethers.formatEther(profitTKB)} TKB`);
}
